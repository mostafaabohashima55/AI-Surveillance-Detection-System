...
function loadUsers() {
  apiRequest('/user/')
    ...
    .catch(err => showToast(err.message || 'Failed to fetch users.', 'danger'))
    .finally(() => hideLoader());
}
...
function markFalseAlert(id) {
  ...
  if (a) { a.status = 'False Alert'; renderAlertsPage(); showToast('Alert marked as false alert', 'warning'); }
}
function markResolved(id) {
  const a = State.alertList.find(x => x.id === id);
  if (a) { a.status = 'Resolved'; renderAlertsPage(); showToast('Alert marked as resolved', 'success'); }
}
...
function renderAdminsPage() {
  ...
  tbody.innerHTML = State.adminList.map(a => `
    <tr>
      ...
      <td>
        <div class="d-flex gap-1">
          <button class="btn-icon" onclick="editAdminRole(${a.id})" title="Edit Role"><i class="fas fa-user-pen"></i></button>
          <button class="btn-icon ${!canDeleteAdmin(currentRole, a.role) ? 'disabled' : 'danger'}" onclick="deleteAdmin(${a.id})"><i class="fas fa-trash"></i></button>
        </div>
      </td>
    </tr>
  `).join('');
...
function normalizeAdminRoles() {
  State.adminList = State.adminList.map(admin => ({
    ...admin,
    role: String(admin.role || 'viewer').toLowerCase().replace(/\s+/g, '_')
  }));
}

function roleLabel(role) {
  return String(role || 'viewer').replace('_', ' ').replace(/\b\w/g, c => c.toUpperCase());
}

function canEditRole(actorRole, targetRole, newRole) {
  if (actorRole === 'super_admin') return true;
  if (actorRole === 'admin') return targetRole === 'viewer' && newRole === 'admin';
  return false;
}
...
function editAdminRole(id) {
  normalizeAdminRoles();
  const admin = State.adminList.find(x => x.id === id);
  if (!admin) return;

  const currentRole = getCurrentUserRole();
  const availableRoles = ['super_admin', 'admin', 'viewer'].filter(r => canEditRole(currentRole, admin.role, r));
...
function saveAdmin() {
  ...
  apiRequest('/user/', {
    method: 'POST',
    body: JSON.stringify({ name, email, password: pass, role: String(role).toLowerCase() })
  })
    .then(() => {
    ...
    .catch(err => showToast(err.message || 'Create user failed.', 'danger'));
}
...
function saveAdminRoleChange() {
  ...
  const admin = State.adminList.find(x => x.id === adminRoleEditingId);
  const currentRole = getCurrentUserRole();
  ...
  apiRequest(`/user/${adminRoleEditingId}`, {
    ...
    .catch(err => showToast(err.message || 'Role update failed.', 'danger'));
}
...
window.editAdmin = editAdminRole;
