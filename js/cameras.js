...
function loadCameras() {
  apiRequest('/camera/')
    .then(res => {
      ...
      State.cameraList = cameras.map(c => ({
        id: c.id || c._id,
        name: c.name || c.cameraName || 'Camera',
        ip: c.rtspUrl || c.ip || c.streamUrl || '-',
        location: c.location || c.zone || '-',
        status: c.isEnabled === false ? 'offline' : 'online',
...
function renderCamerasPage() {
  ...
  const filteredCameras = State.cameraList.filter(c => {
    ...
    return c.name.toLowerCase().includes(searchValue)
      ...
      || c.ip.toLowerCase().includes(searchValue);
  });
...
function saveCamera() {
  ...
  apiRequest(endpoint, {
    method,
    body: JSON.stringify({ name, rtspUrl: ip, location })
  })
    .then(() => {
    ...
    .catch(err => showToast(err.message || 'Camera save failed.', 'danger'));
}
...
function testCamera(id) {
  const cam = State.cameraList.find(c => c.id === id);
  if (!cam) return;
  const nextEnabled = cam.status !== 'online';
  apiRequest(`/camera/${id}/toggle`, {
    method: 'PATCH',
    body: JSON.stringify({ isEnabled: nextEnabled })
  })
    .then(() => {
      cam.status = nextEnabled ? 'online' : 'offline';
      renderCamerasPage();
      showToast(`Camera ${nextEnabled ? 'enabled' : 'disabled'} successfully.`, 'success');
    })
    .catch(err => showToast(err.message || 'Toggle failed.', 'danger'));
}
