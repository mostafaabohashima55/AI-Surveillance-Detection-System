...
function loadDashboardData() {
  Promise.all([apiRequest('/camera/'), apiRequest('/alert/')])
    .then(([camRes, alertRes]) => {
      State.cameraList = asArray(camRes).map(c => ({
        id: c.id || c._id,
      ...
      State.alertList = asArray(alertRes).map(a => ({
        id: a.id || a._id,
        cam: a.cameraName || a.camera?.name || 'Camera',
        camId: a.cameraId || a.camera?.id || null,
        type: a.type || a.alertType || 'Unknown',
        status: a.status || 'Pending',
        time: (a.createdAt || '').replace('T', ' ').slice(0, 19),
        confidence: Number(a.confidence || a.score || 0),
        details: a.details || a.description || ''
      }));
      renderDashboard();
    })
    .catch(err => showToast(err.message || 'Failed to load dashboard data.', 'danger'))
    .finally(() => hideLoader());
}
