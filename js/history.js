...
function loadIncidents() {
  apiRequest('/incident/')
    .then(res => {
      ...
      State.alertList = incidents.map(i => ({
        id: i.id || i._id,
        cam: i.cameraName || i.camera?.name || `Camera ${i.cameraId || ''}`.trim(),
        type: i.type || i.incidentType || 'Incident',
        status: i.status || 'Pending',
        time: (i.createdAt || i.time || '').replace('T', ' ').slice(0, 19),
        confidence: Number(i.confidence || i.score || 0)
      }));
      renderHistoryPage();
    })
    .catch(err => showToast(err.message || 'Failed to fetch incidents.', 'danger'))
    .finally(() => hideLoader());
}
...
function renderHistoryChart() {
  ...
  State.alertList.forEach(a => {
    if (!a.time) return;
    const hour = String(new Date(a.time).getHours()).padStart(2, '0');
